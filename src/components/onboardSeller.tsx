import { SellerForm, Gender, OnboardingProp, User } from "models/model";
...
import { SellerOnboardingStyle } from "style/components/seller-onboarding-style";
const iconStyle = { width: "20px", height: "20px" };
interface SellerOnboardingProp {
  onDone: (s: boolean) => void;
  useSellerState: [
    SellerForm,
    React.Dispatch<React.SetStateAction<SellerForm>>
  ];
...
export const SellerOnboardingForm = ({
  ...
  useIndex,
  useSellerState,
}: SellerOnboardingProp) => {
  const { loadingUpdate, isUpdated, updateSeller } = useUpdateUser();
  ...
  const [state, setState] = useSellerState;
  const [stateError, setStateError] = useState<SellerForm>({} as SellerForm);
  const [gender, setGender] = useState({ is: false, isMale: true });
  const [searchParams, setSearchParams] = useSearchParams();

  const isError = (key: keyof SellerForm) => stateError[key]?.length > 0;
  const setStateFor = (e, key: keyof SellerForm) => {
    setState((s) => {
